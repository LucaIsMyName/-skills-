...
Applies to **retrieval-augmented generation**—answering questions from a specific corpus (docs, policies, wikis). Not full production vector-search engineering (sharding, BM25/hybrid tuning, multi-tenant security). Pair with [`working-with-context-windows.md`](working-with-context-windows.md), [`structured-output-and-tool-use.md`](structured-output-and-tool-use.md), [`security-for-web-apps.md`](../coding/security-for-web-apps.md), and [`evaluating-model-output.md`](evaluating-model-output.md).

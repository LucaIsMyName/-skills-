# Gastbeiträge und Meinungsartikel

**Geltungsbereich:** **Meinungstexte** mit **klarer Urheberschaft**—Aufbau, Offenlegung, Stimme. Kein neutraler Journalismus. Ergänzt [`stories-schreiben.md`](stories-schreiben.md), [`respektvolle-sprache.md`](respektvolle-sprache.md), [`quellenbewertung-und-faktencheck.md`](../recherche-&-analyse/quellenbewertung-und-faktencheck.md) und [`interessenkonflikte-und-offenlegung.md`](../ethik-&-recht/interessenkonflikte-und-offenlegung.md).

## Exzerpt
- **These** früh—dann **Argument** mit Belegen, nicht mit Vibes.
- **Interessen** und **Affiliationen** offenlegen.
- **Fakten**, **Analyse**, **Meinung** trennen; Unsicherheit **kennzeichnen**.
- **Ton**: bestimmt kann **respektvoll** sein—Kritik an **Sachverhalten**, nicht an Personen.
- **Länge** und **Stil** nach Vorgaben des Mediums.

## Bevor du schreibst

### Konkret

- **Zielpublikum**, **Wortlimit**, **Deadline**.
- **Belegbare** Fakten vs. **bewusste** Wertung.

### Meta

- Meinung **überzeugt**—sie **erlaubt** keine Faktenfreiheit.

---

## Zweck

**Glaubwürdige** Meinung veröffentlichen—**Sache** stärken ohne **Integrität** zu opfern.

---

## 1. Struktur

Lede mit Aktualität; Körper mit Belegen; Schluss mit Konsequenz oder Handlungsaufforderung.

## 2. Offenlegung

Finanzierung und Rollen, die die Lesart beeinflussen können.

## 3. Was nicht tun

- Gegner*innen **verzerren** oder **falsch** zitieren.

---

## Kerngedanke
Starker Kommentar = **klare These + ehrliche Belege + transparente Interessen**.

## Weiterführend

- [The OpEd Project](https://www.theopedproject.org/)

---

Englische Version: [`op-eds-and-bylined-articles.md`](../../en/language-&-communication/op-eds-and-bylined-articles.md)
